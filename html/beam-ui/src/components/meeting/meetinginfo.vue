<template>
    <div class="container">
        <div class="handle-box">
            ...
            <el-tag size="medium" effect="dark" type="warning">{{records.meetingStatus}}</el-tag>
        ...
        <el-tabs v-model="initTab" class="collapse">
            <el-tab-pane label="会议信息" name="0">
                <el-collapse v-model="info">
                    ...
                    <el-collapse-item title="会议信息" name="1">
                        ...
                        <div>会议课件：{{records.topicName}}</div>
                ...
                <el-dialog title="填写原因" :visible.sync="noPassVisible" width="400px" center >
                    <el-input v-model="auditorReq.auditorText" type="textarea" :rows="6" placeholder="请输入内容"/>
                        ...
                        <div>现场视频：<br>
                            <el-row :gutter="20">
                                <el-col :span="7" v-for="item in records.meetingVideo" :key="item.id">
                                    <video :src="item.url" width="320" height="240" @click="videoShow(item.url)"></video>
                                </el-col>
                            </el-row>
                        </div>
                        <div>现场照片：<br>
                            <el-row :gutter="20">
                                <el-col :span="7" v-for="item in records.meetingImg" :key="item.id">
                                    <el-image :src="item.url" lazy style="width:320px;height:240px"></el-image>
                                </el-col>
                            </el-row>
                        </div>
                    </el-collapse-item>
            ...
            <el-tab-pane label="会议总结" name="2">
                <el-collapse v-model="conclusion">
                    <el-collapse-item title="会议现场" name="0">
                        <div>劳务报销单：<br>
                            <el-row :gutter="20">
                                <el-col :span="7" v-for="item in records.laborImg" :key="item.id">
                                    <el-image :src="item.url" lazy style="width:300px;height:200px"></el-image>
                                </el-col>
                            </el-row>
                        </div>
                        <div>费用发票：<br>
                        ...
                        <div>会议总结：{{records.conclusion}}</div>
                    </el-collapse-item>
                </el-collapse>
            </el-tab-pane>
            ...
            <el-tab-pane label="审核记录" name="3">
                <el-table :data="tableData" v-loading="loading" border class="table">
                    <el-table-column prop="auditorName" align="center" label="审核人员"></el-table-column>
                    <el-table-column prop="auditorTime" align="center" label="审核时间"></el-table-column>
                    <el-table-column prop="meetingStatus" align="center" label="审核状态"></el-table-column>
                    <el-table-column prop="auditorText" align="center" label="原因"></el-table-column>
                </el-table>
            </el-tab-pane>
        </el-tabs>
    </div>
    ...
    export default {
            ...
            meetingId(id) {
                ...
                this.$api.MeetingDetailApi.getInfo({meetingId:id}).then((res) => {
                    if (res.error === false) {
                        ...
                        this.tableData = res.data.meetingAuditors;
                        if("03" === res.data.meetingStatus){
                            this.passBtn = false;
                        }
        ...
        methods: {
            ...
            auditor(auditorStatus){
                this.auditorReq.meetingId = this.records.id;
                this.auditorReq.auditorStatus = auditorStatus;
                this.$api.MeetingDetailApi.auditor(this.auditorReq).then((res) => {
                    if (res.error === false) {
